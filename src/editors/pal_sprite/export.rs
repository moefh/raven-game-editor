...
impl ExportDialog {
    ...
    pub fn show(&mut self, wc: &mut WindowContext, pal_sprite: &mut PalSprite) {
        if ! self.open { return; }
        if let Some(SysDialogResponse::File(filename)) = wc.sys_dialogs.get_response_for(format!("editor_{}_export_sprite",
                                                                                                 pal_sprite.asset.id)) {
            self.display_filename = Some(filename.as_path().file_name().map(|f| f.display().to_string()).unwrap_or("?".to_owned()));
            self.filename = Some(filename);
        }

        if AssetEditorBase::show_dialog_window(wc, Self::id(), 350.0, "Export Paletted Sprite", |ui, wc| {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if ui.button("...").clicked() {
                                wc.sys_dialogs.save_file(
                                    Some(wc.egui.window),
                                    format!("editor_{}_export_pal_sprite", pal_sprite.asset.id),
                                    "sprite",
