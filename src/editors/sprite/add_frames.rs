...
impl AddFramesDialog {
    ...
    pub fn set_open(&mut self, action: AddFramesAction, sel_tile: u32, sel_color: u8) {
        self.action = action;
        self.num_frames = 1;
    ...
    fn confirm(&mut self, sprite: &mut Sprite) {
        ...
        if matches!(self.action, AddFramesAction::Insert) && self.sel_tile < sprite.num_frames {
            let src_top = self.sel_tile * sprite.height;
            let dst_top = (self.sel_tile + self.num_frames) * sprite.height;
            let row_len = sprite.width as usize;
            let mut src_row = vec![0; row_len];
            let mut dst_row = vec![0; row_len];
            let num_new_rows = (sprite.num_frames - self.sel_tile) * sprite.height;
