...
static GRAY_PAL: [u8; 4] = [ 0b000000, 0b010101, 0b101010, 0b111111 ];
static ALL_PAL: sync::LazyLock<Vec<u8>> = sync::LazyLock::new(create_all_pal);
static GRAD_PAL: sync::LazyLock<Vec<u8>> = sync::LazyLock::new(create_grad_pal);

fn create_all_pal() -> Vec<u8> {
...
fn create_grad_pal() -> Vec<u8> {
    let mut colors = [0u8; 6*5];

    for (c, &grad_color) in GRAD_COLORS.iter().enumerate() {
...
impl ColorPickerWidget {
    ...
    pub fn draw_palette(painter: &egui::Painter, rect: Rect, dims: (i32, i32), palette: &[u8]) {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui, wc: &WindowContext) {
        ...
        Self::draw_palette(&painter, sel_rect, sel_dims, &[self.left_color, self.right_color]);
        Self::draw_palette(&painter, all_rect, all_dims, &ALL_PAL);
        Self::draw_palette(&painter, grays_rect, grays_dims, &GRAY_PAL);
        Self::draw_palette(&painter, grads_rect, grads_dims, &GRAD_PAL);

        if let Some(pointer_pos) = response.interact_pointer_pos() {
            if let Some(index) = Self::check_pick(pointer_pos, all_rect, all_item_size) &&
                let Some(&color) = ALL_PAL.get(index) {
                    self.set_color(color, &response);
                }
            if let Some(index) = Self::check_pick(pointer_pos, grays_rect, grays_item_size) &&
                let Some(&color) = GRAY_PAL.get(index) {
                    self.set_color(color, &response);
                }
            if let Some(index) = Self::check_pick(pointer_pos, grads_rect, grads_item_size) &&
                let Some(&color) = GRAD_PAL.get(index) {
                    self.set_color(color, &response);
                }
        }
    }
