...
use crate::misc::colors::raven_color_to_rgb;
...
const GRAD_PAL8: [u8; 78] = [
    ...
    0b00_001_000, 0b00_010_000, 0b01_011_000, 0b01_100_000, 0b10_101_000, 0b10_110_000,
    0b11_111_000,
    ...
    0b00_000_001, 0b00_000_010, 0b01_000_011, 0b01_000_100, 0b10_000_101, 0b10_000_110,
    0b11_000_111,
    ...
    fn show(&mut self, ui: &mut egui::Ui, wc: &WindowContext, id: egui::Id) {
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                Self::draw_palette(&painter, response.rect, (2, 1), &[self.state.left_color, self.state.right_color]);
                ...
                let maybe_inner = egui::containers::Popup::menu(&response)
                    ...
                    .show(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            Self::draw_palette(&painter, response.rect, (32, 8), &ALL_PAL8);
                        ...
                        egui::Frame::NONE
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    let h_color_block_size = COLOR_BLOCK_SIZE * 13.0 / 6.0;
                                    ...
                                    Self::draw_palette(&painter, draw_rect, (num_colors, 1), &GRAY_PAL8);
                                    if let Some(pos) = response.interact_pointer_pos() && response.rect.contains(pos) {
                                ...
                                // gradients palette
                                ui.horizontal(|ui| {
                                    let alloc_size = Vec2::new(32.0 * COLOR_BLOCK_SIZE - 5.0, 6.0 * COLOR_BLOCK_SIZE);
                                    let (response, painter) = ui.allocate_painter(alloc_size, Sense::click_and_drag());
                                    ...
                                    Self::draw_palette(&painter, draw_rect, (13, 6), &GRAD_PAL8);
                                    if let Some(pos) = response.interact_pointer_pos() && response.rect.contains(pos) {
