    ...
    pub fn show(&mut self, ui: &mut egui::Ui, samples: &[i16], loop_start: &mut f32, loop_end: &mut f32, height: f32) {
        ...
        if self.samples_per_point >= 1.0 {
            for i in 0 .. (canvas_rect.width().ceil() as usize) {
                let start_index = ((self.first_sample + i as f32 * self.samples_per_point).round() as usize).min(samples.len()-1);
                let end_index = ((self.first_sample + (i+1) as f32 * self.samples_per_point).round() as usize).min(samples.len()-1);
