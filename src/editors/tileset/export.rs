...
impl ExportDialog {
    ...
    pub fn set_open(&mut self, tileset: &Tileset) {
        self.filename = None;
        self.display_filename = None;
        self.num_items_x = (tileset.num_tiles as f32).sqrt().ceil() as u32;
        self.open = true;
    }
    ...
    pub fn show(&mut self, wc: &mut WindowContext, tileset: &mut Tileset) {
        if ! self.open { return; }
        if let Some(SysDialogResponse::File(filename)) = wc.sys_dialogs.get_response_for(format!("editor_{}_export_tileset", tileset.asset.id)) {
            self.display_filename = Some(filename.as_path().file_name().map(|f| f.display().to_string()).unwrap_or("?".to_owned()));
            self.filename = Some(filename);
        }

        if egui::Modal::new(egui::Id::new("dlg_tileset_export")).show(wc.egui.ctx, |ui| {
            ui.set_width(300.0);
            ui.with_layout(egui::Layout::top_down_justified(egui::Align::Center), |ui| {
                ui.heading("Export Tileset");
                ui.separator();
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui.button("...").clicked() {
                                    wc.sys_dialogs.save_file(Some(wc.egui.window), format!("editor_{}_export_tileset", tileset.asset.id),
                                                             "Export Tileset",
                                                             &[
                                                                 ("PNG files (*.png)", &["png"]),
                                                                 ("All files (*.*)", &["*"]),
                            ...
                            ui.add(egui::Slider::new(&mut self.num_items_x, 1..=tileset.num_tiles));
                            ui.end_row();
                        });
                });
