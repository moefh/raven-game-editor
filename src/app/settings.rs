...
pub struct AppSettings {
    ...
    pub game_runner_default_zoom: u32,
...
impl AppSettings {
    ...
    fn save_gamepad_mapping(id: &str, mapping: &GamepadMapping) -> String {
        fn save(id: &str, mapping: &GamepadMapping) -> std::result::Result<String,std::fmt::Error> {
            ...
            writeln!(&mut out, "  \"{}\" = [", id)?;
            writeln!(&mut out, "    [")?;
            for (raw, button) in mapping.buttons.iter() {
                writeln!(&mut out, "      0x{:x},0x{:x},", raw, button)?;
            }
            writeln!(&mut out, "    ],")?;
            writeln!(&mut out, "    [")?;
            for (raw, axis) in mapping.axes.iter() {
                writeln!(&mut out, "      0x{:x},0x{:x},0x{:x},", raw, axis.min, axis.max)?;
            }
            writeln!(&mut out, "    ]")?;
            writeln!(&mut out, "  ],")?;
            Ok(out)
    ...
    pub fn save(&self, logger: &mut StringLogger) {
        ...
        config.push_str(&format!("game_runner_default_zoom = {};\n", self.game_runner_default_zoom));
...
impl<'a> AppSettingsReader<'a> {
    ...
    fn read_gamepad_button_mapping_array(&mut self) -> Result<HashMap<u32, u32>> {
        ...
        loop {
            ...
            if let Some(raw) = t.get_number() {
                let raw = (raw & u32::MAX as u64) as u32;
                self.expect_punct(',')?;
                let btn = self.read_number()? as u32;
                buttons.insert(raw, btn);
            }
        }
        Ok(buttons)
    }
    ...
    fn read_gamepad_mappings_config(&mut self) -> Result<HashMap<String, GamepadMapping>> {
        ...
        loop {
            ...
            if let Some(id) = t.drain_string() {
                self.expect_punct('=')?;
                self.expect_punct('[')?;
                let buttons = self.read_gamepad_button_mapping_array()?;
                self.expect_punct(',')?;
        ...
        loop {
            ...
            if let TokenData::Ident(ident) = t.data {
                ...
                match ident.as_str() {
                    ...
                    "game_runner_default_zoom" => { settings.game_runner_default_zoom = self.read_number_config()?; }
