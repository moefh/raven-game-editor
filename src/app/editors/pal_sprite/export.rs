...
impl ExportDialog {
    ...
    fn confirm(&mut self, file: SysDialogOpenFile, wc: &mut WindowContext, pal_sprite: &mut PalSprite) -> bool {
        if let Err(e) = pal_sprite.save_image_png(self.num_items_x).and_then(|data| file.write_data(data)) {
            wc.open_message_box("Error Exporting", format!("Error exporting pal sprite to {}:\n{}", file.filename(), e));
            false
        } else {
            true
        }
