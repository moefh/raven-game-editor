...
impl ModListDiff {
    ...
    pub fn compare(&mut self, cur_store: &DataAssetStore, other_store: &DataAssetStore) {
        ...
        self.cur_only.splice(.., cur_store.asset_ids.mods.iter().copied());
        for other in other_store.assets.mods.iter() {
            self.cur_only.retain(|cur_id| ! cur_store.assets.mods.get(cur_id).is_some_and(|cur| cur.asset.name == other.asset.name));
            if let Some(cur) = super::get_mod_by_name(cur_store, &other.asset.name) {
                if let Some(diff) = ModDiff::compare(cur, other) {
...
impl ModDiff {
    pub fn compare(cur: &ModData, other: &ModData) -> Option<Self> {
        let cur_hash = calc_asset_data_hash(cur);
        let other_hash = calc_asset_data_hash(other);
        if cur_hash != other_hash {
            Some(ModDiff { cur_id: cur.asset.id, other_id: other.asset.id })
        } else {
            None
        }
    }
