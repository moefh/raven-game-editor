...
impl AppTextureManager {
    ...
    */
    ...
    pub fn get_rgba_texture(&mut self, ctx: &egui::Context, name: AppTextureName,
                            w: usize, h: usize, data: &[u32], force_load: bool) -> &egui::TextureHandle {
        let stride = w.div_ceil(4usize);
        let load_image = || {
            let mut pixels = Vec::<egui::Color32>::with_capacity(4 * w * h);
            for y in 0..h {
                for (i, &quad) in data[y*stride..(y+1)*stride].iter().enumerate() {
                    let mut src = quad;
                    let n = if (i+1)*4 <= w { 4 } else { (i+1)*4 - w };
                    for _p in 0..n {
                        ...
                        if r==0 && g==3 && b == 0 {
                            ...
                            let cr = ((r << 6) | (r << 4) | (r << 2) | r) as u8;
                            let cg = ((g << 6) | (g << 4) | (g << 2) | g) as u8;
                            let cb = ((b << 6) | (b << 4) | (b << 2) | b) as u8;
                            pixels.push(egui::Color32::from_rgb(cr, cg, cb));
                        }
                    }
                }
            }
